...
- 2026-09-24 DIJIANG 3.7: resolving the upstream-owned `changelog/unreleased.md` through `git checkout --theirs` in the restricted shell could not create `.git/index.lock`. Retry that exact conflict-resolution command with scoped elevated execution; leave the changelog body to upstream as required by the repository's release-note rule.
- 2026-09-24 DIJIANG 3.7: `rg` was given a literal `packages/ui/src/lib/i18n/messages/*.ts` path in PowerShell and Windows rejected the wildcard as a path. Search the existing directory directly, or use `rg -g '*.ts'`, rather than relying on shell glob expansion.
- 2026-09-24 DIJIANG 3.7: a UI type-check log path used `../../../.codex-temp` from `packages/ui`, which resolves outside the repository; PowerShell could not create it, so the reported exit was not a valid check. Use `../../.codex-temp` or an absolute workspace path and rerun the command.
- 2026-09-24 DIJIANG 3.7: automatic approval rejected a `git add` command that dynamically collected all unstaged tracked paths. Verify the exact changed paths and stage them as explicit literal file lists; preserve unrelated and untracked work.
- 2026-09-24 DIJIANG 3.7: `bunx vitest` could not access Bun's temporary directory in the restricted sandbox (`EPERM`), and workspace-wide Bun scripts could not start child processes. Rerun only the same focused validation with scoped escalation; do not change global Bun settings.
- 2026-09-24 DIJIANG 3.7: Node's native `--test` first failed to spawn in the restricted sandbox; after scoped escalation it could not resolve the extensionless TypeScript import in the VS Code test. Keep that package's isolated Bun runner as the source of truth rather than treating the native Node attempt as a product failure.
- 2026-09-24 DIJIANG 3.7: the local VS2022 toolset lacks Spectre-mitigated libraries, so `node-pty` native rebuild failed with MSB8040 after web staging and OpenCode CLI verification succeeded. For local portable QA only, back up the installed `node_modules` binding file and remove its Spectre property; restore it after packaging. The committed source and GitHub CI configuration remain unchanged.
- 2026-09-24 DIJIANG 3.7: an optional CIM process-command-line query returned Access Denied while the portable build was running. Use the build log, scoped process status and exit code; do not retry broader process inspection.
- 2026-09-24 DIJIANG 3.7: the first portable CDP probe hit a target navigation and the second wrapper launch exited before extraction. Attach to the stable renderer with retry, and separate wrapper startup from the unpacked app's runtime check. Stop only processes under the task's isolated profile.
- 2026-09-24 DIJIANG 3.7: a long QA probe could not save a screenshot because H: had no free space. The first portable run had left a 1.97 GB task-owned extraction directory; verify no process uses it and its resolved path is inside `.codex-temp` before removing only that directory. The disk then had about 2 GB free. Avoid repeating full portable extraction in the constrained workspace.
