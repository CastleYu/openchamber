...
import { fetchAgentV2Permissions, writeAgentV2, type AgentV2Effect as PermissionEffect, type AgentV2Rule as PermissionRule } from './agentV2Config';
import {
  SettingsSection,
  SettingsChipGroup,
...
interface AgentPermissionsEditorProps {
  agent: Extract<AgentWithExtras, { generation: 'oc2' }>;
  /**
...
export const AgentPermissionsEditor: React.FC<AgentPermissionsEditorProps> = ({
  ...
  React.useEffect(() => {
    ...
    setLoadFailed(false);
    void (async () => {
      const envelope = await fetchAgentV2Permissions(agentName, settingsDirectory);
      if (cancelled) return;
      if (!envelope) {
        setLoadFailed(true);
        setIsLoading(false);
  ...
  const save = React.useCallback(async (): Promise<AutosaveResult> => {
    ...
    try {
      await writeAgentV2('PATCH', agentName, { permissions: serializeRules(model) }, settingsDirectory);
    } catch {
  ...
  }, [agentName, baseline, model, settingsDirectory, t]);
