...
import { selectAgentsForDirectory, useAgentsStore, type AgentScope, type AgentWithExtras } from '@/stores/useAgentsStore';
import { fetchAgentV2Entity, writeAgentV2, type AgentV2Config as AgentConfig, type AgentV2Entity as AgentEntity, type AgentV2Envelope as AgentEntityEnvelope } from './agentV2Config';
...
import { AgentPermissionsEditor } from './AgentPermissionsEditorV2';
...
export const AgentsPageV2: React.FC = () => {
  const { t } = useI18n();
  const providers = useConfigStore((state) => state.providers).filter((provider) => provider.generation === 'oc2');
  const {
    selectedAgentName,
    getAgentByName,
    agentDraft,
  ...
  const selectedAgent: Extract<AgentWithExtras, { generation: 'oc2' }> | null = candidate?.generation === 'oc2' ? candidate : null;
  ...
  const [sourceStatus, setSourceStatus] = React.useState<'idle' | 'loading' | 'ready' | 'failed'>('idle');
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    if (agentDraft.sourceAgentName) {
      setSourceStatus('loading');
      void fetchAgentV2Entity(agentDraft.sourceAgentName, settingsDirectory).then((envelope) => {
        if (cancelled) return;
        entityRef.current = envelope.config;
        const source = envelope.config;
        const parsed = parseModelSelection(source.model);
        setDescription(source.description || '');
        setMode(source.mode || 'subagent');
        setModel(parsed ? `${parsed.providerID}/${parsed.modelID}` : '');
        setVariant(parsed?.variant || '');
        setSteps(source.steps ?? undefined);
        setTemperature(source.request?.body?.temperature);
        setTopP(source.request?.body?.top_p);
        setSystem(source.system || '');
        setSourceStatus('ready');
      }).catch((error) => {
        if (!cancelled) {
          setSourceStatus('failed');
          toast.error(error instanceof Error ? error.message : t('settings.agents.page.toast.saveUnexpectedError'));
        }
      });
    } else setSourceStatus('idle');
    ...
    if (!agentDraft.sourceAgentName) entityRef.current = {};
    setStoredAt(null);
  ...
  }, [agentDraft, isNewAgent, settingsDirectory, t]);
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    void (async () => {
      const envelope = await fetchAgentV2Entity(selectedAgentName, settingsDirectory);
  ...
  }, [isNewAgent, selectedAgent, selectedAgentName, settingsDirectory, selectionKey]);
  ...
  const buildConfig = React.useCallback((agentName: string): AgentConfig => {
    ...
    const request: NonNullable<AgentEntity['request']> = {};
    ...
    // at creation time.
    if (isNewAgent && (agentDraft?.permissions?.length || entityRef.current.permissions?.length)) {
      config.permissions = agentDraft?.permissions ?? entityRef.current.permissions;
    }
    return config;
  ...
  const save = React.useCallback(async (): Promise<AutosaveResult> => {
    ...
    try {
      await writeAgentV2('PATCH', agentName, config, settingsDirectory);
    } catch {
  ...
  const handleCreate = async () => {
    if (agentDraft?.sourceAgentName && sourceStatus !== 'ready') return;
    ...
    setIsCreating(true);
    try {
      await writeAgentV2('POST', agentName, buildConfig(agentName), settingsDirectory);
      setAgentDraft(null);
      ...
      {isNewAgent && (
        <div className="flex items-center gap-2 pb-8">
          <Button
            ...
            disabled={isCreating || !draftName.trim() || Boolean(agentDraft?.sourceAgentName && sourceStatus !== 'ready')}
            size="xs"
