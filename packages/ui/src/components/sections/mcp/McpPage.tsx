...
export const McpPage: React.FC = () => {
  ...
  const runtimeFailureError = runtimeStatus?.status.status === 'failed'
    ? runtimeStatus.status.error
    : undefined;
  const runtimeFailureHint = runtimeStatus?.status.status === 'failed'
            ...
            <div className={cn('rounded-lg border p-3', statusCardClass(effectiveStatusName))}>
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {runtimeStatus?.status.status === 'failed' && (
                    <Button
                      variant="ghost"
                      size="xs"
                      className="!font-normal gap-1 text-muted-foreground"
