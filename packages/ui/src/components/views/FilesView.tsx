...
import { buildFileTreeStatusIndex } from './files/fileTreeStatus';
...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full', visible = true }) => {
  ...
  React.useEffect(() => {
    if (!visible || !selectedFile?.path || loadedFilePath !== selectedFile.path || fileError) {
      return;
    }
  ...
  }, [applyLoadedTextContent, contentDetectedBinary, fileError, loadedFilePath, readFile, readFileStat, selectedFile?.path, t, visible]);
  ...
  const canUseShikiFileView = isTextFile && !isMarkdown && !isPlantUml && !isDrawio
  ...
  const isEditingFile = (isMarkdown && mdViewMode === 'edit')
    ...
    || (!isMarkdown && !isPlantUml && !isHtml && !isJson && !isSelectedSvg && !isMermaid && !isTable && textViewMode === 'edit');
  ...
  const mdPreviewFocusTargetPath = selectedFile && (isMarkdown || isPlantUml)
    && (isMarkdown ? getMdViewMode() : getPlantumlViewMode()) === 'preview' && !fileLoading
    ? selectedFile.path
    : null;
  React.useEffect(() => {
  ...
  const pdfAssetAuthKey = selectedFile?.path
    && usesRawAssetUrl && (!files.loadAsset || isSelectedFont)
  ...
  const renderArtifactPreview = (file: FileNode): React.ReactNode => {
    const personalKind = mediaKind(file.path);
    if (personalKind && !isSvgSource && (files.loadAsset || (personalKind === 'zip' && files.archiveEntries))) {
      return <MediaPreview key={file.path} path={file.path} options={selectedFileReadOptions} />;
    }
  ...
  const renderFloatingFileControls = ({
    ...
    return (
      <div className={wrapperCls}>
        ...
        {!isSelectedNonText && (!isPlantUml || plantumlViewMode === 'edit') && (
          <>
