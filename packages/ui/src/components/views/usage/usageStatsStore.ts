...
/** The OpenChamber backend and its selected OpenCode connection both own Stats. */
export const getUsageStatsRuntimeKey = (): string => {
  const runtime = opencodeClient.getBoundRuntime();
  return JSON.stringify([
    getRuntimeKey(), runtime?.generation ?? null, runtime?.endpoint ?? null,
    runtime?.epoch ?? null, runtime?.version ?? null,
  ]);
};
...
export const useUsageStatsStore = createUsageStatsStore(fetchFromOpencode, getUsageStatsRuntimeKey);
