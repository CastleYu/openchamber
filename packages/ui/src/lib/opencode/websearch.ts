...
import type { IntegrationInfo, OpenCodeClient } from "@opencode/client"
import { z } from "zod"
...
const clientFor = (directory: string | null): OpenCodeClient => {
  ...
  if (runtime?.generation !== 'oc2') throw new OpenCodeRuntimeError(runtime?.generation ?? 'unknown', 'websearch')
  return createV2RuntimeClient({
...
async function call<T>(operation: string, run: () => Promise<T>): Promise<T> {
  const runtime = opencodeClient.getBoundRuntime()
  if (runtime?.generation !== 'oc2') throw new OpenCodeRuntimeError(runtime?.generation ?? 'unknown', operation)
...
export async function saveWebSearchKey(providerId: string, key: string): Promise<void> {
  await call("integration.connect.key", () =>
    clientFor(null).integration.connect.key({ integrationID: providerId, key }),
  )
}
