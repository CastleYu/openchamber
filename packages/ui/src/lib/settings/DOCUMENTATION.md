...
- `updateHistory.ts` — read-only history classification and page/search identities.
  ...
  Each bullet is grouped by the functional category after its source/version
  prefix; translated category names may differ while the English and Chinese
  entries remain in the same order and none are dropped.
