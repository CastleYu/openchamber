...
const entryKey = (directory: string, source: WalkthroughSource): string =>
  `${getRuntimeKey()}${ENTRY_SEP}${directory}${ENTRY_SEP}${walkthroughSourceKey(source)}`;

const loadKeyPrefixForDirectory = (directory: string): string =>
  `${getRuntimeKey()}${ENTRY_SEP}${directory}${ENTRY_SEP}`;
...
interface WalkthroughActions {
  ...
  abortLoad: (directory: string, source: WalkthroughSource) => void;
  abortLoadsForDirectory: (directory: string) => void;
...
// putting it in the store would make every abort a re-render. Load and generate
// must not share a controller: hiding the tab aborts the GET, not a paid job.
const loadInFlight = new Map<string, AbortController>();
const generateInFlight = new Map<string, AbortController>();
  ...
  devtools(
    (set, get) => ({
      ...
      load: async (directory, source, options = {}) => {
        ...
        if (current?.status === 'generating' || generateInFlight.has(key)) return;

        loadInFlight.get(key)?.abort();
        const controller = new AbortController();
        loadInFlight.set(key, controller);
        ...
        try {
          ...
          if (loadInFlight.get(key) === controller) loadInFlight.delete(key);
        }
      },
      ...
      generate: async (directory, source, options = {}) => {
        ...
        generateInFlight.get(key)?.abort();
        const controller = new AbortController();
        generateInFlight.set(key, controller);
        ...
        try {
          ...
          stopPolling();
          if (generateInFlight.get(key) === controller) generateInFlight.delete(key);
        }
      },

      abortLoad: (directory, source) => {
        const key = entryKey(directory, source);
        const controller = loadInFlight.get(key);
        if (!controller) return;
        controller.abort();
        loadInFlight.delete(key);
      },

      abortLoadsForDirectory: (directory) => {
        const prefix = loadKeyPrefixForDirectory(directory);
        for (const [key, controller] of loadInFlight) {
      ...
      reset: () => {
        for (const controller of loadInFlight.values()) controller.abort();
        loadInFlight.clear();
        for (const controller of generateInFlight.values()) controller.abort();
        generateInFlight.clear();
