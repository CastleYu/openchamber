...
const currentDirectory = (): string | null => requestedDirectory === undefined
  ? opencodeClient.getDirectory()?.trim() || null : requestedDirectory;

export const getWebSearchScopeKey = (directory = currentDirectory()): string => JSON.stringify([getRuntimeKey(), opencodeClient.getBoundRuntime(), directory]);
...
export const useWebSearchStore = create<WebSearchStore>()((set, get) => ({
  ...
  load: async (selectedDirectory) => {
    if (selectedDirectory !== undefined) requestedDirectory = selectedDirectory;
    ...
    try {
      ...
      const [providers, config] = await Promise.all([
        ...
        opencodeClient.getTaggedConfig(directory),
      ]);
      if (config.generation !== 'oc2') return;
      ...
      const selection = localChoiceIsNewer && current.kind === 'ready' && current.snapshot.scope === scope
        ...
        : readWebSearchSelection(config.value);
  ...
  setSelection: async (selection) => {
    const current = get().state;
    if (current.kind !== 'ready' || current.snapshot.projectOverride || current.snapshot.scope !== getWebSearchScopeKey() || opencodeClient.getBoundRuntime()?.generation !== 'oc2') return false;
    ...
    if (getWebSearchScopeKey() !== current.snapshot.scope) return false;
    if (!failed) {
  ...
  saveKey: async (providerId, key) => {
    const current = get().state;
    if (current.kind !== 'ready' || current.snapshot.scope !== getWebSearchScopeKey()) return false;
    try {
    ...
    if (current.snapshot.scope !== getWebSearchScopeKey()) return false;
    void get().load();
    return true;
  },
