...
- A bootstrap holds its scheduler slot through critical state and the authoritative directory session-list fetch. Deferrable LSP/VCS/question/permission enrichment starts afterward without extending slot ownership or competing with the initial session-list request. MCP status and commands are loaded by their foreground feature stores because OpenCode command discovery also starts MCP servers.
- `useSessionListSync` remains the demand owner for the current and selected directories; background-eligible topology is published only for directories that satisfy the eligibility predicate. Known projects and worktrees do not bootstrap merely because they are shown or restored. Sidebar notices still request bootstrap manually with `force`.
- A directory never bootstrapped reads as `ready` in sidebar group status; denied access surfaces when selected. Bootstrap list loading and environment initialization remain separate, and initialization failures preserve list data and expose retry.
- Every initialization read explicitly selects its directory. Status, question, and permission recovery start independently of configuration and optional LSP/VCS enrichment; failed reads preserve prior data. MCP status and command discovery remain foreground-demand only.
- Session pages and background reads share their bounded request lanes, and pagination retries do not replay successful pages.
...
Child-session discovery and active-session watchdog recovery use the shared background network lanes, with live status authority kept separate from the global session cache.
