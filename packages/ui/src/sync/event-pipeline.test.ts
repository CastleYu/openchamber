...
function createSdk(events: StreamPayload[], streamFinished: () => void, onYield?: () => void): OpenCodeClient {
...
describe("createEventPipeline", () => {
  test("background streaming publishes fewer batches without losing text", async () => {
    const run = async (focused: boolean) => {
      ...
      let text = ""
      const pipeline = createEventPipeline({
        sdk: createSdk(Array.from({ length: 60 }, () => textDelta("x")), finish, () => advance(4)),
        transport: "sse",
        now: () => clock,
        flushFrameMs: 0,
        flushTimer: {
