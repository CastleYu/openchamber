...
describe('readCredentialsFromDb', () => {
  it('returns the active credential per integration and null without a database', () => {
    // Bun 1.4 exposes node:sqlite but retains a Windows file handle after
    // DatabaseSync.close(). Exercise the on-disk fixture only under Node.
    if (process.versions.bun || !sqlite) {
