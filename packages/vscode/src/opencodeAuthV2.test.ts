...
import { projectCredentialValue, readCredentialsFromDb, resolveCredentialDbPath } from './opencodeAuthV2';
...
describe('readCredentialsFromDb', () => {
  it('returns the active credential per integration and null without a database', async () => {
    ...
    try {
      ...
      // Bun on Windows can release node:sqlite's file handle after close()
      // returns; keep the fixture cleanup bounded while preserving EBUSY.
