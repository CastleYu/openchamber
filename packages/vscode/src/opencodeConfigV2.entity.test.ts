...
let configModule: Promise<typeof import('./opencodeConfigV2')> | undefined;
const loadConfig = () => (configModule ??= import('./opencodeConfigV2'));
...
describe('VS Code entity modules speak OpenCode 2 shapes', () => {
  ...
  test('writes plugins while the OC1 config module still writes plugin', async () => {
    const { createPluginEntry } = await loadConfig();
    const configPath = path.join(process.env.XDG_CONFIG_HOME!, 'opencode', 'opencode.json');
    createPluginEntry({ spec: 'example@1.0.0', scope: 'user' }, projectDir);
    const saved = readJson(configPath);
    assert.deepEqual(saved.plugins, ['example@1.0.0']);
    assert.equal('plugin' in saved, false);
  });
