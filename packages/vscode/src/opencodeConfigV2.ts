import { OPENCODE_CONFIG_DIR_V2 as OPENCODE_CONFIG_DIR } from './opencodeConfigPaths';
...
export type PluginEntry = {
  ...
  /** The actual config file that declares the entry. */
  sourcePath: string;
};
...
const encodePluginId = (prefix: 'config' | 'file' | 'package' | 'legacy', value: string): string =>
...
export const listPluginDirFiles = (workingDirectory?: string): PluginFile[] => {
  ...
  for (const scope of ['user', 'project'] as const) {
    ...
    try {
      preferred = getPluginDir(scope, workingDirectory);
    } catch {
      continue;
    }
    for (const dir of [preferred, path.join(path.dirname(preferred), 'plugin')]) {
      if (!fs.existsSync(dir)) continue;
      const editable = dir === preferred;
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        if (entry.name.startsWith('.') || entry.name.includes('..')) continue;
        const absolutePath = path.join(dir, entry.name);
        if (entry.isDirectory()) {
          files.push({ id: encodePluginId('package', `${scope}:${entry.name}`), fileName: entry.name, scope, kind: 'package', absolutePath });
          continue;
        }
        if (entry.isFile() && PLUGIN_FILE_NAME_PATTERN.test(entry.name)) {
          files.push({
            id: encodePluginId(editable ? 'file' : 'legacy', `${scope}:${entry.name}`),
            fileName: entry.name, scope, kind: editable ? 'file' : 'package', absolutePath,
          });
        }
      }
    }
