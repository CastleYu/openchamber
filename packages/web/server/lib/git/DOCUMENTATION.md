...
- `getUntrackedDiffs(directory, filePaths, options)`: Diffs for untracked files against an empty tree. Resolves repository context once per batch. The occupancy policy bounds concurrency, file count, and individual file bytes; binary and oversized files are skipped. Selected input and retained output each have a 32 MiB aggregate ceiling. Returns one entry per input path in order; omitted paths yield `''`. The caller's abort signal stops child processes, and the promise settles after those workers finish. Limits apply per batch.
