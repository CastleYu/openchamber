...
const isFilenameTooLongError = (message) => {
  ...
  return /file ?name too long/i.test(text)
    || (/cannot create directory at/i.test(text) && /invalid argument/i.test(text));
};
...
const runPostCheckoutHook = async (directory) => {
  ...
  // Let Git invoke hooks with its own platform shell and hooksPath handling.
  // Spawning a shebang script directly fails with ENOENT on Windows.
  const result = await runGitCommand(directory, ['hook', 'run', '--ignore-missing', 'post-checkout', '--', GIT_NULL_REF, head, '1']);
  if (!result.success) {
    // A failing hook must not fail worktree creation or session bootstrap.
    console.warn(`[GitService] post-checkout hook failed in worktree ${directory}: ${result.message}`);
  }
};
