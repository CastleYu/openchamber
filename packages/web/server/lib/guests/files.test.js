...
describe('resolveGuestFilePath', () => {
  ...
  test('accepts an outside path only when a declared pattern covers its canonical form', async () => {
    ...
    const declared = await resolveGuestFilePath({
      ...
      patterns: ['~/.config/opencode/opencode.json', `${guestPath(probe)}/**`],
      homeDir: home,
    });
    expect(declared).toEqual({ ok: true, scope: 'filesystem', absolute: path.join(home, '.config', 'opencode', 'opencode.json') });
    const below = await resolveGuestFilePath({ path: `${guestPath(probe)}/a/b.txt`, projectDirectory: null, patterns: [`${guestPath(probe)}/**`], homeDir: home });
    ...
    const none = await resolveGuestFilePath({ path: `${guestPath(probe)}/a`, projectDirectory: null, patterns: [], homeDir: home });
  ...
  test('matches through a symlinked pattern prefix and refuses a symlink out of the granted tree', async () => {
    ...
    const viaLinkPattern = await resolveGuestFilePath({ path: `${guestPath(link)}/x.txt`, projectDirectory: null, patterns: [`${guestPath(link)}/**`], homeDir: holder });
    ...
    const escape = await resolveGuestFilePath({ path: `${guestPath(real)}/escape/y.txt`, projectDirectory: null, patterns: [`${guestPath(real)}/**`], homeDir: holder });
...
describe('runGuestFileOperation', () => {
  ...
  test('needs files for the project and filesystem for outside', async () => {
    ...
    expect(await run({ op: 'stat', path: `${guestPath(probe)}/x`, grants: ['files'] })).toMatchObject({ ok: false, code: 'NOT_GRANTED' });
  ...
  test('writes atomically, creating parents, and refuses oversized content', async () => {
    ...
    expect(await run({ op: 'write', path: guestPath(target), content: '{"a":1}', fsPromises })).toEqual({ ok: true, result: { written: true } });
    ...
    expect(await run({ op: 'write', path: guestPath(target), content: '{"a":2}', fsPromises })).toEqual({ ok: true, result: { written: true } });
    ...
    expect(await run({ op: 'write', path: guestPath(target), content: 'x'.repeat(GUEST_FILE_CONTENT_MAX + 1) })).toMatchObject({ ok: false, code: 'FILE_TOO_LARGE' });
    expect(await fs.readFile(target, 'utf8')).toBe('{"a":2}');
  });
  ...
  test('lists sorted entries with kinds and caps the list', async () => {
    ...
    const capped = await run({ op: 'list', path: guestPath(many), patterns: [`${guestPath(many)}/**`] });
