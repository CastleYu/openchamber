...
beforeAll(async () => {
  ...
  if (process.platform === 'win32') {
    await fs.writeFile(path.join(bin, 'ssh.cmd'), `@echo off\r\n"${process.execPath}" "${path.join(bin, 'ssh')}" %*\r\n`);
    setEnv('GIT_SSH', path.join(bin, 'ssh.cmd'));
  }
  // Seed only this explicit fixture file. Never run `git config --global` here.
  const sshCommand = process.platform === 'win32' ? `'${path.join(bin, 'ssh.cmd').replaceAll('\\', '/')}'` : 'ssh';
  await fs.writeFile(path.join(root, '.gitconfig'), `[user]\n  name = Fixture\n  email = fixture@example.com\n[core]\n  sshCommand = ${JSON.stringify(`${sshCommand} -i '${globalKey}'`)}\n`);
...
describe('SSH extension installs', () => {
  ...
  test('persists the selected identity and re-resolves it for update checks and replacement clones', async () => {
    ...
    expect((await sshCalls()).at(-1).args).toContain(process.platform === 'win32' ? workKey.replaceAll('\\', '/') : workKey);
    ...
    expect((await sshCalls()).at(-1).args).toContain(process.platform === 'win32' ? rotatedKey.replaceAll('\\', '/') : rotatedKey);
