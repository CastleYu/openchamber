...
export const registerNotificationEmitRoutes = (app, dependencies) => {
  const { express, isAgentToolRequestAuthorized, emitter, getKernelRuntime } = dependencies;

  const jsonBody = express.json({ limit: '16kb' });

  const respond = async (req, res, next) => {
    try {
      const generation = getKernelRuntime().generation;
      if (generation !== 'oc2') {
        return res.status(generation === 'oc1' ? 501 : 503).json({ error: 'Plugin notifications require OpenCode 2' });
      }
