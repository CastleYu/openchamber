...
const createApp = ({ settings = { nativeNotificationsEnabled: true, notificationMode: 'hidden-only' }, now = () => 0, generation = 'oc2' } = {}) => {
  const delivered = [];
  const app = express();
...
describe('POST /api/notifications/emit', () => {
  it('keeps the new plugin notification endpoint unavailable on OC1', async () => {
    const { app, delivered } = createApp({ generation: 'oc1' });
    const response = await request(app).post('/api/notifications/emit')
      .set('Authorization', UI_TOKEN).send({ title: 'Done' });
