...
describe('settings helpers', () => {
  ...
  it('imports from the packed @openchamber/web tarball without escaping the published package', async () => {
    ...
    try {
      ...
      execFileSync(resolveBunExecutable(), ['pm', 'pack', '--filename', join(packDir, 'web.tgz')], {
        cwd: packagesWebDir,
        stdio: 'pipe',
      });
...
describe('settings registry gate', () => {
  ...
  const validValues = {
    occupancy: { idlePollingEnabled: true },
    ...
    agentControlToolEnabled: true, agentWebToolEnabled: true, agentNotifyToolEnabled: true, browserProvider: 'builtin', agentMemoryToolEnabled: true, openCodeUpdateToastDismissedVersion: '1.0.0',
