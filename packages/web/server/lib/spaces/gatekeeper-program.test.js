...
// Some host DNS services synthesize a local answer for reserved `.test` names.
// Either result must be refused before a connection leaves the gatekeeper.
const NO_ROUTE_DECISION = /^deny:(?:unresolved:|blocked-address$)/;
...
describe('gatekeeper program', () => {
  ...
  describe('corridor, in allowlist mode', () => {
    ...
    it('resolves a name itself, and refuses it when nothing comes back', async () => {
      // Reserved names may fail resolution or resolve to a blocked local address.
      expect((await tunnelTo('blocked.test:443')).status).toMatch(/403/);
      expect(await decisionFor('blocked.test')).toMatchObject({ listener: 'corridor', port: 443, decision: expect.stringMatching(NO_ROUTE_DECISION) });
    });
  ...
  describe('corridor, in open mode', () => {
    ...
    it('tells the space refused from unreachable, and nothing else about why', async () => {
      ...
      expect((await decisionFor('blocked.test')).decision).toMatch(NO_ROUTE_DECISION);
    });
    ...
    it('resolves the name itself before it decides anything about the address', async () => {
      ...
      expect(await decisionFor('blocked.test', at)).toMatchObject({ decision: expect.stringMatching(NO_ROUTE_DECISION) });
    });

    it('allows port 443 only here too, so a space can attack nothing else from the user\'s address', async () => {
      // A reserved name is refused after the name and port rules, whether DNS
      // rejects it or supplies a blocked local address.
      // have passed. Every other port is refused before any of that.
      const at = await journalMark();
      await tunnelTo('blocked.test:443');
      expect(await decisionFor('blocked.test', at)).toMatchObject({ port: 443, decision: expect.stringMatching(NO_ROUTE_DECISION) });
  ...
  describe('control channel', () => {
    ...
    it('records a connection that was allowed and did not happen, apart from a refusal', async () => {
      ...
      // Record a refusal after the name rules, independent of host DNS policy.
      // The failure record is covered where a host does resolve: the escape suite.
      expect(decision.decision).toMatch(NO_ROUTE_DECISION);
    }, 30_000);
